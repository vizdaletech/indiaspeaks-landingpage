    ...
    width={100}
        ...
        <Link
          ...
          <p
            ...
            />
              <div className={`w-[250px] ${isDropOpen ?  `h-40` : `h-0 `} transition-all duration-300 overflow-hidden  dropdown z-[100]  bg-primary text-white flex flex-col gap-6 items-start justify-center absolute right-0 top-[50px] text-base`}>
                <Link
                  href="/services/semiconductor-services"
                    className={`border-b border-transparent ml-6 ${isDropOpen ? `translate-y-0 opacity-1` : `-translate-y-full opacity-0`} transition-all duration-300 delay-300 `}
                >
                  Semiconductor Services
                </Link>
                <Link
                  href="/services/system-solutions"
                  className={`border-b border-transparent ml-6 ${isDropOpen ? `translate-y-0 opacity-1` : `-translate-y-full opacity-0`} transition-all duration-200 delay-[400ms]`}
                >
                  Systems Solution
                </Link>
                <Link
                  href="/services/technology-services"
                  className={`border-b border-transparent ml-6 ${isDropOpen ? `translate-y-0 opacity-1` : `-translate-y-full opacity-0`} transition-all duration-200 delay-500`}
                >
                  Technology Services
                </Link>
              </div>
